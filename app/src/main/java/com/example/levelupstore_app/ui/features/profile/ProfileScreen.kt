...
private sealed class ProfileScreen(val route: String, val label: String, val icon: ImageVector) {
    object Data : ProfileScreen(Screen.ProfileData.route, "Mis Datos", Icons.Default.Person)
    object Orders : ProfileScreen(Screen.OrderHistory.route, "Mis Pedidos", Icons.Default.List)
}
private val profileNavItems = listOf(ProfileScreen.Data, ProfileScreen.Orders)
...
fun ProfileScreen(
    ...
    // 2. Creamos un NUEVO NavController SÓLO para esta sección
    val profileNavController = rememberNavController()

    Scaffold(
        // 3. Barra de Navegación Inferior (para "Mis Datos" y "Mis Pedidos")
        bottomBar = {
            NavigationBar {
                    ...
                    NavigationBarItem(
                        ...
                        onClick = {
                            profileNavController.navigate(screen.route) {
                                // Navegación estándar para barras inferiores
        ...
        // 4. Icono para Cerrar Sesión en la barra superior (opcional pero útil)
        topBar = {
            TopAppBar(
